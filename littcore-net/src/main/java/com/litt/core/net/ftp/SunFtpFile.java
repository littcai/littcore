...
public class SunFtpFile {
	
	private static final Log logger = LogFactory.getLog(SunFtpFile.class);
	
    private FtpClient ftpclient;
	private String host;      //主机名
	private int port = FtpClient.FTP_PORT;         //端口
	private String userName;      //用户名
	private String password;  //密码 

    /**
     * 构造函数
     * @param host String 机器IP
     * @param port String 机器FTP端口号
     * @param userName String FTP用户名
     * @param password String FTP密码
     * @throws IOException
     */
    public SunFtpFile(String host, int port, String userName, String password) throws IOException 
    {
        this.host = host;
        this.port = port;           
    ...
    /**
     * 构造函数
     * @param host String 机器IP，默认端口为21
     * @param userName String FTP用户名
     * @param password String FTP密码
     * @throws IOException
     */
    public SunFtpFile(String host,String userName, String password) throws IOException {
        this.host = host; 
        this.userName = userName;
        this.password = password;
    }
    ...
    public int logout() throws IOException {
    	...
    	{
    		logger.error("closeServer()方法断开FTP出错，调用关闭命令",e);
    		try
    		{
        	...
        	{
    			logger.error("调用关闭命令方式断开FTP出错",e1);
        	}
    	}
...
//        ftpOut.flush();
    ...
    public static void main(String[] args) throws Exception {
        SunFtpFile fUp = new SunFtpFile("10.9.73.134", 21, "administrator", "boco123");
        fUp.login();
