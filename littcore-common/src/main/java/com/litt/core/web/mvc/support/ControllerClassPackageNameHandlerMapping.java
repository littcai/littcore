...
package com.litt.core.web.mvc.support;
 ...
 */
public class ControllerClassPackageNameHandlerMapping extends AbstractControllerUrlHandlerMapping {
	 ...
	 * 按控制器所在包名进行URL模块化.
	 */
	private boolean packageModule = true;
	...
	public void setPackageModule(boolean packageModule) {
	...
	public void setPathPrefix(String prefixPath) {
		...
		if (!StringUtils.isEmpty(this.pathPrefix)) {
			if (this.pathPrefix.length() == 0 || this.pathPrefix.charAt(0) != '/') {
	...
	public void setBasePackage(String basePackage) {
		...
		if (!StringUtils.isEmpty(this.basePackage) && !this.basePackage.endsWith(".")) {
	...
	private StringBuilder buildPathPrefix(Class beanClass) {
		...
		{
			String packageName = ClassUtils.getPackageName(beanClass);
			//将类型中的Web前的包名作为URL前缀，以支持模块化
			String[] packageNames = StringUtils.split(packageName, '.');
			String subPackage = packageNames[packageNames.length-2];
			pathMapping.append(this.caseSensitive ? subPackage : subPackage.toLowerCase());
			pathMapping.append('/');
		}
		return pathMapping;
