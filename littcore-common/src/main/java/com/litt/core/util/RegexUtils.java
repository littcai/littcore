...
{	
	 ...
	 * 模式映射缓存.
	 */
	private static final Map patternCacheMap = new HashMap();
	
	/**
	 * 模式映射缓存.
	 * 不区分大小写
	 */
	private static final Map patternSoftCacheMap = new HashMap();	
     ...
     * 匹配非负整数（正整数 + 0)
     */
    public static final String NON_NEGATIVE_INT_REGEXP = "^\\d+$";

    /**
     * 匹配不包括零的非负整数（正整数 > 0)
     */
    public static final String NON_ZERO_NEGATIVE_INT_REGEXP = "^[1-9]+\\d*$";
     ...
     *
     */
    public static final String POSITIVE_INT_REGEXP = "^[0-9]*[1-9][0-9]*$";
     ...
     *
     */
    public static final String NON_POSITIVE_INT_REGEXP = "^((-\\d+)|(0+))$";
     ...
     *
     */
    public static final String NEGATIVE_INT_REGEXP = "^-[0-9]*[1-9][0-9]*$";
     ...
     *
     */
    public static final String INT_REGEXP = "^-?\\d+$";
     ...
     *
     */
    public static final String NON_NEGATIVE_FLOATTING_POINT_NUMBERS_REGEXP = "^\\d+(\\.\\d+)?$";
     ...
     *
     */
    public static final String POSITIVE_FLOATTING_POINT_NUMBERS_REGEXP = "^(([0-9]+\\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\\.[0-9]+)|([0-9]*[1-9][0-9]*))$";
     ...
     *
     */
    public static final String NON_POSITIVE_FLOATTING_POINT_NUMBERS_REGEXP = "^((-\\d+(\\.\\d+)?)|(0+(\\.0+)?))$";
     ...
     *
     */
    public static final String NEGATIVE_FLOATTING_POINT_NUMBERS_REGEXP = "^(-(([0-9]+\\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\\.[0-9]+)|([0-9]*[1-9][0-9]*)))$";
     ...
     *
     */
    public static final String FLOATTING_POINT_NUMBERS_REGEXP = "^(-?\\d+)(\\.\\d+)?$";
     ...
     *
     */
    public static final String CAPITAL_LETTER_REGEXP = "^[A-Z]+$";
     ...
     *
     */
    public static final String PIC_REGEXP = "^(/{0,1}\\w){1,}\\.(gif|bmp|png|jpg)$|^\\w{1,}\\.(gif|bmp|png|jpg)$";
   ...
   private static Pattern getPattern(String regexp) throws MalformedPatternException
   {
	   if(patternCacheMap.containsKey(regexp))
		   return (Pattern)patternCacheMap.get(regexp);
	   else 
	   {
		   Pattern pattern = new Perl5Compiler().compile(regexp);
		   patternCacheMap.put(regexp, pattern);
		   return pattern;
	   }
   }
   
   private static Pattern getSoftPattern(String regexp) throws MalformedPatternException
   {
	   if(patternSoftCacheMap.containsKey(regexp))
		   return (Pattern)patternSoftCacheMap.get(regexp);
	   else 
	   {
		   Pattern pattern = new Perl5Compiler().compile(regexp, Perl5Compiler.CASE_INSENSITIVE_MASK);
		   patternSoftCacheMap.put(regexp, pattern);
		   return pattern;
	   }
   } 
    ...
    public static boolean validate(String source, String regexp)
    {
        ...
        {            
            // 实例大小大小写敏感的正规表达式模板
            Pattern hardPattern = getPattern(regexp);
            // 返回批配结果
            return new Perl5Matcher().contains(source, hardPattern);
        }
	 ...
	 * @return 匹配返回true,否则返回false
	 */
    public static boolean validateSoft(String source, String regexp)
    {
        try
        {
            //实例不区分大小写的正规表达式模板
            Pattern softPattern = getSoftPattern(regexp);            
            //返回批配验证值
            return new Perl5Matcher().contains(source, softPattern);
        }
        catch (MalformedPatternException e)
        {
        	return false;
     ...
     *              matchResult.group(4) = /index.html?login=true
     */
    public static MatchResult getSoftMatchResult(String source, String regexp)
    {
        try
        {
            // 实例不区分大小写的正规表达式模板
     ...
     */
    public static List getMatchList(String source, String regexp)
    {
     ...
     * @return 如果源字符串符合要求返回许要的批配结果集{@link #getHardRegexpMatchResult(String, String) 使用方法参见getHardRegexpMatchResult(String, String)}
     */
    public static List getSoftMatchList(String source, String regexp)
    {
    	List tempList = new ArrayList();
        try
        {
   ...
   *              如       String source = "((([a]+[b])/[c])-24)+[d]";
   *              则          getExpression(originStr,"[","]"));
   *                           返回结果集合有四个元素  [a, b, c, d],
   *                          以 ASCII 的升序排列
   * </pre>
   *
   ...
   *                    指定分隔符中间的字符串的集合
   */
  public static List<String> getExpression(String source,String leftSeparator,String rightSeparator)
  {
      List<String> expList = new ArrayList<String>();
      String sTempArray[] = source.split("(\\"+leftSeparator+")");
  ...
  public static String escapeExprSpecialWord(String keyword) {  
    ...
    {
    	//System.out.println(RegexUtils.validate("littcai@hotmail.com", RegexUtils.EMAIL_REGEXP));
    	//System.out.println(RegexUtils.validate("2005-04-04", RegexUtils.DATE_REGEXP));
    	//RegexUtils.validate("123", RegexUtils.ID_CARD_REGEXP);
    	
    	//String expression = "({controlDomain}.length + {addressDomain}.length + {chainDataDomain}.length)";
    	//List list = RegexUtils.getExpression(expression, "{", "}");
    	
    	String expression = "(,\"controlDomain\".length + ,\"addressDomain\".length + ,chainDataDomain,.length)";
    	List<String> list = RegexUtils.getExpression(expression, ",\"", "\"");
    	...
    	List<String> result = RegexUtils.getExpression("\"aa=bb\"", "\"", "\"");
